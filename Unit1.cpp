...
void onswitch(bool ch, bool en, bool vi)
{
    ...
    Form1->RadioButton3->Checked=ch;         //w³¹cznik pól
...
//---------------------------------------------------------------------------
void __fastcall TForm1::Button1Click(TObject *Sender) //Sprawdza odpowiedŸ, punktuje. Przechodzi do nastêpnego
{                                                     //Klawisz >>
    ...
    if(OpenDialog1->Tag==1)
    {
        ...
        RadioButton2->Caption=Utf8ToAnsi(Memo1->Lines->Strings[i+2]);     //Wczytanie nowych pytañ
    ...
    else
    {
        ...
        RadioButton2->Caption=Memo1->Lines->Strings[i+2];     //Wczytanie nowych pytañ
...
{                                                   //Sprawdza odpowiedŸ, zaznacza poprawn¹
  Button2->Enabled=false;
...
{
if(OpenDialog1->Execute())                           //Otwórz z pliku i rozpocznij quiz
{
  try
  {
    ...
    i=0;
    points=0;
    Memo1->Lines->LoadFromFile(OpenDialog1->FileName);   //za³adowanie treœci quizu z pliku
    RadioButton1->Color=clBtnFace;
    RadioButton2->Color=clBtnFace;      //reset kolorków
    RadioButton3->Color=clBtnFace;
    RadioButton4->Color=clBtnFace;
    Label1->Caption=Utf8ToAnsi(Memo1->Lines->Strings[0]);         //Utf8ToAnsi() dla polskich znaków
...
void menuclick(TMemo *quiz)
{
    i=0;
    points=0;
    Form1->Memo1->Lines=quiz->Lines;   //za³adowanie treœci quizu z memo
    Form1->RadioButton1->Color=clBtnFace;
    Form1->RadioButton2->Color=clBtnFace;      //reset kolorków
...
{
                                             //Otwórz z Spoleczna1 i rozpocznij quiz
